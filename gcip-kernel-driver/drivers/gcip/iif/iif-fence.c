...
/* Checks whether @fence is unblocked or not. */
static bool iif_fence_is_unblocked_locked(struct iif_fence *fence)
{
	lockdep_assert_held(&fence->signaled_signalers_lock);

	return iif_fence_is_signaled_locked(fence) || fence->signal_error;
}
 ...
 * Caller must hold @fence->signaled_signalers_lock.
 */
static int iif_fence_signal_locked(struct iif_fence *fence)
{
	lockdep_assert_held(&fence->signaled_signalers_lock);
 ...
 *
 * This function must be called only if @fence is unblocked so that @fence->signaled_signalers_lock
...
static void iif_fence_destroy(struct kref *kref)
{
	...
	if (!list_empty(&fence->poll_cb_list) && !iif_fence_is_unblocked_locked(fence))
	 ...
	 * - If `iif_fence_is_unblocked_locked(fence)` was true, it means that the fence was already
...
int iif_fence_signal_with_status(struct iif_fence *fence, int error)
{
	...
	ret = iif_fence_signal_locked(fence);
	if (ret < 0)
		goto out;

	/* We should set the error only when signaling the fence has been succeeded. */
	iif_fence_set_signal_error_locked(fence, error);
	unblocked = iif_fence_is_unblocked_locked(fence);
out:
	spin_unlock_irqrestore(&fence->signaled_signalers_lock, flags);
...
int iif_fence_get_signal_status(struct iif_fence *fence)
{
	...
	if (fence->signal_error)
		status = fence->signal_error;
	else
		status = iif_fence_is_signaled_locked(fence);
				...
				iif_fence_poll_cb_t func)
{
	...
	if (iif_fence_is_unblocked_locked(fence)) {
